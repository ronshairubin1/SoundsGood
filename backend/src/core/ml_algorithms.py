...
class RFModel(ModelBase):
    ...
    def _load_model(self, path):
        try:
            # Try loading with joblib first (preferred method)
            if path.endswith('.joblib'):
                self.model = joblib.load(path)
                logging.info(f"Loaded RF model from joblib file: {path}")
                return True
                
            # For .pkl extension, try joblib first then fall back to pickle for legacy support
            elif path.endswith('.pkl'):
                try:
                    self.model = joblib.load(path)
                    logging.info(f"Loaded RF model from pickle file using joblib: {path}")
                    return True
                except Exception as joblib_error:
                    logging.warning(f"Joblib failed to load pickle file, trying pickle: {joblib_error}")
                    # Fall back to pickle for legacy support
                    import pickle
                    with open(path, 'rb') as f:
                        self.model = pickle.load(f)
                    logging.info(f"Loaded RF model from pickle file using pickle module: {path}")
                    return True
            else:
                # For files without extension, try joblib
                try:
                    self.model = joblib.load(path)
                    logging.info(f"Loaded RF model without extension using joblib: {path}")
                    return True
                except Exception as joblib_error:
                    logging.warning(f"Failed to load with joblib, error: {joblib_error}")
                    return False
...
def create_model(model_type, model_dir='models'):
    ...
    if model_type.lower() == 'cnn':
        ...
        return RFModel(model_dir=model_dir)
